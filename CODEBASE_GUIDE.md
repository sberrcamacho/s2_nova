...
1. **Pages** (`src/dashboard/pages/`) — the v2 destinations: Inicio,
   Movimientos, Planes (tabs Presupuestos · Metas · Préstamos, hosting
   `BudgetsPage`/`GoalsPage`/`components/LoansTab`), Reportes, Ajustes
   (+ `pages/ajustes/*` sub-views). Pre-v2 paths redirect in `routes.tsx`;
   see `web/AGENTS.md`'s Information Architecture section. These compose
   UI components and read data via hooks/services.
2. **Shared components**
   ...
   - `src/components/v2/` — the v2 mockup's building blocks
     (`CategoryMark`, `Money`, icons, `DetailDialog`, and `Kit.tsx`: the
     centered modal shell, labels, flat pills, `ConfirmDialog`)
3. **App-wide state** (`src/state/`) — React Context, not Redux:
   ...
   - `useCurrency` / `useTranslation` — hooks bound to the user's
     principal currency and `user.preferences.language`; **always** go
     through these instead of importing `lib/currency.ts` directly or
     hardcoding copy.
4. **Local dashboard state**: `src/dashboard/usePeriod.ts` — the month
   selector (`?period=YYYY-MM`) shared by the header and Movimientos.
5. **API layer** (`src/services/` + `src/lib/apiClient.ts`) —
   `services/*.ts` (e.g. `transactionService.ts`, `summaryService.ts`)
   ...
   category registry (taxonomy dotted id ↔ backend UUID; ids come from
   `lib/taxonomy.json`, generated by `scripts/gen-taxonomy.mjs`). Per the
   functional-parity rule (root `AGENTS.md`), Web writes everything
   Android writes; each write is a thin call and the backend applies the
   side effects.
...
1. **Routes** (`src/routes/`) — `auth.ts` (register/login/refresh/logout +
   ...
   `categories.ts`, `currencies.ts`, `transactions.ts`, `budgets.ts`,
   `goals.ts`, `recurringSeries.ts`, `summary.ts`, `alerts.ts`,
   `security.ts`, `dataExport.ts`, `health.ts`. Every route except `auth/*`,
...
2. **Schema** (`prisma/schema.prisma`) — source of truth for the DB shape;
   `prisma/seed.ts` seeds the global (`user_id = null`) category taxonomy
   from `src/lib/taxonomy.json` (`slug` = the node's dotted id); the same
   JSON is generated into Web and Android by `scripts/gen-taxonomy.mjs`.
